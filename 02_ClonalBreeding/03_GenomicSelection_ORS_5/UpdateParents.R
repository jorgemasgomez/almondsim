...
if (year >= nBurnin+1) {
  ...
  while (added < max_new_parents && length(ECT1_temp@id) > 0) {
    ...
    ECT1_temp=setEBV(ECT1_temp, gsmodel, value = "bv")
    cand <- selectInd(ECT1_temp, nInd = 1, candidates = ECT1_temp@id, use = "ebv")
    ...
    if (length(ECT1_temp_ids) > 0) {
      ...
      ECT1_temp <- selectInd(ECT1_temp, nInd = length(ECT1_temp_ids), candidates = ECT1_temp_ids, use = "pheno") #It does not affect
    } else {
      break
    }
  }
